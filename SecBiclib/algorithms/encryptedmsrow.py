...
class ClacEncMSRow:
    ...
    def calculate_msr_col_addition(self, HE, cipher_data, cipher_data_cols, data_size):
        ...
        cipher_col_mean = self.col_mean(HE, cipher_data, data_size)
        ...
        if isinstance(cipher_data, list):
            for i in range(len(cipher_data)):
                ...
                HE.rescale_to_next(cipher_data_cols[i])
        ...
        if isinstance(cipher_data, list):
            ...
            cipher_row_residue = cipher_data_cols - cipher_row_mean - cipher_col_mean + cipher_data_mean
            cipher_row_square_residue = cipher_row_residue ** 2
            HE.rescale_to_next(cipher_row_square_residue)
            cipher_row_msr = self.row_mean(HE, ~cipher_row_square_residue, data_size)
            HE.rescale_to_next(cipher_row_msr)

            cipher_row_inverse_residue = -cipher_data_cols + cipher_row_mean - cipher_col_mean + cipher_data_mean
